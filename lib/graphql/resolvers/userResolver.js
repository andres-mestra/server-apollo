...
module.exports = {
  /* User:{
    posts: async (parent, { pages }, ctx )=>{
      ...
      //let keys = await ctx.loaders.test.load(parent.id);
      let result = await ctx.models.post.findAll({
        where:{ authotId: parent.id}
      }, 
      {[ctx.loadSequelize.expected]: ctx.loadSequelize.context});
      
      let prime = await ctx.loadSequelize.context.prime(result);
      console.log({result});
      return result;
    }
  }, */ 
